...
export default ({ database, className }: Props) => {
    ...
    const schema = object().shape({
        confirm: string()
            ...
            .oneOf([database.name.split('_', 2)[1], database.name], 'データベース名を提供する必要があります。'),
    });
        ...
        <>
            <Formik onSubmit={submit} initialValues={{ confirm: '' }} validationSchema={schema} isInitialValid={false}>
                {({ isSubmitting, isValid, resetForm }) => (
                    ...
                    >
                        ...
                        <p css={tw`text-sm`}>
                            データベースの削除は永続的なアクションであり、元に戻すことはできません。これにより、
                            <strong>{database.name}</strong> データベースが完全に削除され、関連するデータがすべて削除されます。
                        </p>
                        <Form css={tw`m-0 mt-6`}>
                            <Field
                                ...
                                name={'confirm'}
                                label={'データベース名を確認'}
                                description={'削除を確認するには、データベース名を入力します。'}
                            />
            ...
            <GreyRowBox $hoverable={false} className={className} css={tw`mb-2`}>
                ...
                <div css={tw`ml-8 text-center hidden md:block`}>
                    ...
                    <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`}>からの接続</p>
                </div>
                <div css={tw`ml-8 text-center hidden md:block`}>
                    <CopyOnClick text={database.username}>
