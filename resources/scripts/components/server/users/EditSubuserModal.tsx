    ...
    return (
        <Formik
            ...
            validationSchema={object().shape({
                email: string()
                    .max(191, 'メールアドレスは191文字を超えてはなりません。')
                    .email('有効なメールアドレスを入力する必要があります。')
                    .required('有効なメールアドレスを入力する必要があります。'),
                ...
                {!isRootAdmin && loggedInPermissions[0] !== '*' && (
                    <div css={tw`mt-4 pl-4 py-2 border-l-4 border-cyan-400`}>
                        <p css={tw`text-sm text-neutral-300`}>
                            アカウントに現在割り当てられている権限のみが、他のユーザーを作成または変更するときに選択できます。
                        </p>
                    </div>
                )}
                {!subuser && (
                    <div css={tw`mt-6`}>
                        <Field
                            ...
                            label={'ユーザーメールアドレス'}
                            description={
                                'このサーバーのサブユーザーとして招待したいユーザーのメールアドレスを入力します。'
                            }
                        />
                    </div>
                )}
