...
<div class="row">
    ...
    <div class="col-sm-4">
        <div class="box box-success">
            <div class="box-header with-border">
                <h3 class="box-title">自動構築</h3>
            </div>
            <div class="box-body">
                <p class="text-muted small">
                    下のボタンを使用すると、ターゲットのサーバー上でwingsを一つのコマンドで
                    設定するためのコマンドを生成できます。
                </p>
            </div>
            <div class="box-footer">
    ...
    $('#configTokenBtn').on('click', function (event) {
        $.ajax({
            ...
            swal({
                type: 'success',
                title: 'トークンが作成されました。',
                text: '<p>ノードを自動設定するには以下のコマンドを実行してください:<br /><small><pre>cd /etc/pterodactyl && sudo wings configure --panel-url {{ config('app.url') }} --token ' + data.token + ' --node ' + data.node + '{{ config('app.debug') ? ' --allow-insecure' : '' }}</pre></small></p>',
                html: true
            })
        }).fail(function () {
            swal({
